...
public class TestUtils {
  ...
  public static JsonNode loadFile(String resource) {
    try (InputStream stream = TestUtils.class.getClassLoader().getResourceAsStream(resource)) {
      ...
      if (resource.endsWith(".json")) return jsonMapper.readTree(reader);
      else if (resource.endsWith(".yaml")) return yamlMapper.readTree(reader);
      else throw new JsltException("Unknown format: " + resource);
    } catch (IOException e) {
