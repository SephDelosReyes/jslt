...
public class ArraySlicer extends AbstractNode {
  ...
  public JsonNode apply(Scope scope, JsonNode input) {
    ...
    if (sequence.isTextual()) size = sequence.asString().length();
    ...
    if (sequence.isArray()) {
      ...
      for (int ix = leftix; ix < rightix; ix++) result.add(sequence.get(ix));
      return result;
    } else {
      String string = sequence.asString();
  ...
  public ExpressionNode optimize() {
    if (left != null) this.left = left.optimize();
    if (right != null) this.right = right.optimize();
